...
let wordsList = (str, subStr) => {
  let a = new RegExp('\\.|,|\\?|!|:|;|"', 'gui');
  let array = str.replace(a, '').toLowerCase().split(' ').filter((arrayItem) => arrayItem.indexOf(subStr) > -1);
  let result = new Set();
...
let getLocalDate = (date, isSeconds, isISO) => {
  ...
  let result;
  if (!isISO) {
    result = isSeconds ? date.toLocaleString() : date.toLocaleString().replace(reg, '');
  } else {
...
let Car2 = function (engine, model, name, year, used) {
  this.engine = engine; 
  this.model = model;
  this.name = name;
  this.year = year;
...
let car = new Car(2000, '6', 'Mazda', 2005, 'used');
let car2 = new Car(3500, 'x7', 'BMW', 2022, 'new');
